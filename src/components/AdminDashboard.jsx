        ...
        <div className="dashboard-layout">
            ...
            <aside className="dashboard-sidebar">
                ...
                <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <li onClick={() => setActiveTab('overview')} style={{ cursor: 'pointer', color: activeTab === 'overview' ? 'var(--primary)' : 'var(--text-secondary)', fontWeight: activeTab === 'overview' ? '600' : 'normal' }}>Overview</li>
                    ...
                    <>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                                {[
                                    { title: 'Total Users', value: stats.totalUsers, color: 'var(--primary)' },
                                    { title: 'Total Leaves', value: stats.totalLeaves, color: 'var(--success)' },
                                    { title: 'Total Reimbursements', value: stats.totalReimbursements, color: 'var(--warning)' },
                                    { title: 'Pending User Requests', value: stats.activeRequests, color: 'var(--danger)' }
                                ].map((stat, idx) => (
                        ...
                        <div className="chart-grid">
                            <div className="card">
                                ...
                                <div style={{ height: 300 }}>
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={overviewData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" label>
                            ...
                            <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <span style={{ color: 'var(--text-secondary)' }}>More charts coming soon</span>
                            </div>
                        </div>
                    </>
                )}
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        {userRequests.filter(r => r.status === 'Pending').map((req) => (
                                            <tr key={req._id}>
                                                <td style={{ fontSize: '0.9rem' }}>{new Date(req.createdAt).toLocaleDateString()}</td>
                                                <td>
                                                    <div><strong>{req.manager?.name}</strong></div>
                                                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{req.manager?.email}</div>
                                                </td>
                                                <td>
                                                    <span className={`badge ${req.action === 'Add' ? 'badge-approved' : 'badge-rejected'}`}>{req.action}</span>
                                                </td>
                                                <td>
                                                    {req.action === 'Add' ? (
                                                ...
                                                <td>
                                                    <div className="flex gap-2">
                                                        <button onClick={() => handleRequestAction(req._id, 'Approved')} className="btn btn-success" style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem' }}>Approve</button>
                                                        <button onClick={() => handleRequestAction(req._id, 'Rejected')} className="btn btn-danger" style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem' }}>Reject</button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                                    ...
                                    <tbody>
                                        {allUsers.map((user) => (
                                            <tr key={user._id}>
                                                <td>{user.name}</td>
                                                <td>{user.email}</td>
                                                <td><span className="badge badge-approved">{user.role}</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="card" style={{ height: 'fit-content' }}>
                            ...
                            <form onSubmit={handleAddUser}>
                                ...
                                <div className="form-group mb-4">
                                    <label>Role</label>
                                    <select required value={newUserForm.role} onChange={e => setNewUserForm({ ...newUserForm, role: e.target.value })}>
